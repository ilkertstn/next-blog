  ...
  return (
    <div className="flex flex-col h-auto p-0 mb-10 tablet:p-10 tablet:mb-0 justify-start items-center w-[100%] tablet:w-[50%]  basis-1/3  group ">
      <div
        ...
        className="w-full h-[300px] relative drop-shadow-2xl tablet:hover:scale-105 tablet:ease-in duration-300 cursor-pointer"
      >
        <Image
      ...
      <div className="flex flex-col w-full items-start ml-2">
        <div className="font-roboto text-grey tiny pt-4">Posted   {time}</div>
        <div
          onClick={() => router.push(`/blog/${slug}`)}
          className="font-roboto h-[20px] font-bold pt-2 w-64 dark:text-white text-soDarkGrey group-hover:underline group-focus:underline cursor-pointer"
        >
        ...
        <div className="pt-4">
          <button
            onClick={() => router.push(`/blog/${slug}`)}
            className="font-roboto font-bold text-red200 h6"
          >
            {read}
          </button>
        </div>
